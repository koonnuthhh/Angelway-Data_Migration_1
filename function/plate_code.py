...
df = pd.read_excel(r"C:\Users\Acer\Desktop\Data migration\Tem.3_WM\zfloan50 ใหม่.XLSX",usecols = 'AN',dtype= str)
df.columns = ['เลขทะเบียน']
df['เลขทะเบียน'] = df['เลขทะเบียน'].fillna('')  # Replace NaN with empty string

Andrew = prepare(df)

Andrew.to_excel('Nut.xlsx')